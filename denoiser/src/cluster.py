...
def cluster(radius, templates, picDic, improvSNR):
    ...
    backplotwindow = np.exp(-((4*np.maximum(0, (xv-0.5))
                            ** 2-0.1)+(4*np.maximum(0, (yv-0.5))**2-0.1)))
    if (len(templates[0].shape) > 2):
        backplotwindow = np.repeat(
            backplotwindow[..., None], templates[0].shape[2], axis=2)
    ...
    for jt in range(len(templates)):
        ...
        templateShape_reshaped = int(
            picDic[jt][0]["template"].shape[0]*picDic[jt][0]["template"].shape[1])
        if (len(templates[0].shape) > 2):
            templateShape_reshaped = int(
                picDic[jt][0]["template"].shape[0]*picDic[jt][0]["template"].shape[1]*picDic[jt][0]["template"].shape[2])
        ...
        if (len(templates[0].shape) > 2):
            variance = [np.zeros((2*radius, 2*radius, templates[0].shape[2]))
        ...
        for j in range(len(clusters_)):
            variance[clusters_[
                j]-1] += deepcopy((picDic[jt][j]["template"] - centroid[clusters_[j]-1])**2)
...
def backplotFinal(centroidDic, picDic, imgs, radius, templateMatchingResults):
    ...
    if (len(imgs.shape) > 3):
        backplotwindow = np.repeat(
            backplotwindow[..., None], imgs.shape[3], axis=3)
    ...
    for jt in range(len(centroidDic)):
        for jc in range(len(centroidDic[jt])):
            for j in centroidDic[jt][jc]["id"]:
                ...
                n = backplotwindow + \
                    overlayCount[myindex][x:(x+2*radius), (y):(y+2*radius)]
                old_avg = (overlay[i][x:(x+2*radius), (y):(y+2*radius)]) / (overlayCount[i][x:(x+2*radius), (y):(
                    y+2*radius)] + (np.double(overlayCount[i][x:(x+2*radius), y:(y+2*radius)] == 0)))
                delta = centroidDic[jt][jc]["centroid"] - old_avg
                overlayM2[i][x:(x+2*radius), (y):(y+2*radius)] = overlayM2[i][x:(x+2*radius), (y):(y+2*radius)] + \
                    centroidDic[jt][jc]["variance_M2"] + delta**2*backplotwindow * \
                    overlayCount[myindex][x:(x+2*radius), (y):(y+2*radius)]
                overlayVariance[i][x:(x+2*radius), (y):(y+2*radius)] = overlayM2[i][x:(
                    x+2*radius), (y):(y+2*radius)]/(n + (n == 0))

                overlay[myindex][x:(x+2*radius), (y):(y+2*radius)
                                 ] += centroidDic[jt][jc]["centroid"]*backplotwindow
                overlayCount[myindex][x:(x+2*radius),
                                      (y):(y+2*radius)] += backplotwindow
    ...
    for i in range(len(imgs)):
        imgBackplots.append(
            overlay[i] / (overlayCount[i] + (np.double(overlayCount[i] == 0))))

        try:
            mymin.append(np.min(imgBackplots[i][imgBackplots[i] > np.min(
                imgBackplots[i][imgBackplots[i] > 0])]))
