...
def tempfuncname(radius, image, templates, maxNumberInClass, minNumberInClass):
    ...
    maxresultindices, maxresults, sortedIndices = classifyTemplates(
        radius, image, templates)

    newTemplates = generateNewTemplates(
        templates, image, sortedIndices, maxresults, maxresultindices, radius, maxNumberInClass, minNumberInClass)
...
def backplotImg(radius, imgs, templates):
    ...
    if (len(imgs[0].shape) > 2):
        backplotwindow = np.repeat(
            backplotwindow[..., None], imgs[0].shape[2], axis=2)
    ...
    for i in range(len(sortedIndices)):
        ...
        for j in range(idxd.shape[1]):
            ...
            overlay[i][idxd[0][j]:(idxd[0][j]+2*radius), (idxd[1][j]):(
                idxd[1][j]+2*radius)] += deepcopy(templates[templateIdx]*backplotwindow)
            overlayCount[i][idxd[0][j]:(
                idxd[0][j]+2*radius), (idxd[1][j]):(idxd[1][j]+2*radius)] += backplotwindow
            n += 1
            # except:
            #     pass
