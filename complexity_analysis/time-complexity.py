...
pixels = np.array([128*128, 256*256, 500*600, 500*800,
                  625*800, 600*1024, 800*1024, 1024*1024])
...
templateSize = np.array([15, 18, 21, 24, 27, 30])
templateSize = templateSize*2

pixels = np.array([128*128, 256*256, 500*600, 500*800,
                  625*800, 600*1024, 800*1024, 1024*1024])

t = np.array([45.8, 31.00, 25.8, 21.83, 18.53, 16.51])

# plotting the points
plt.plot(templateSize, t, '-bo')
plt.xlabel('Reference patch size')
plt.ylabel('Time in secs')

plt.savefig(
    './analyse/tempSizevstime.png')
